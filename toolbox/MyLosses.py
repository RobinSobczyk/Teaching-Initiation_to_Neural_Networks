...
class MyCrossEntropyLoss(nn.CrossEntropyLoss):
    ...
    def init(
        ...
        super(MyCrossEntropyLoss).__init__(
...
class MyHDL(_Loss):
    ...
    def __init__(
        ...
        self.centerloss = CenterLoss(
            hierarchy_size[0], output_filter, truth_filter, device
        )
        self.crossentropies = nn.ModuleList(
            [
                nn.CrossEntropyLoss(
                    weight=auto_arg(weight, i),
    ...
    def forward(
        self, input: Sequence[torch.Tensor], target: torch.Tensor
    ) -> torch.Tensor:
        loss_value = auto_arg(self.lambdas, 0) * self.centerloss(input[0], target[:, 0])
        for i in range(1, self.hierarchy_length):
            loss_value += auto_arg(self.lambdas, i) * self.crossentropies[i - 1](
                input[i], target[:, i]
            )
        return loss_value
...
class MyScatteredHL(_Loss):
    ...
    def forward(
        ...
        loss_value = auto_arg(self.lambdas, 0) * self.crossentropies[0](
            input[0], target[:, 0]
        )
        for i in range(1, self.hierarchy_length):
            loss_value += auto_arg(self.lambdas, i) * self.crossentropies[i](
                input[i], target[:, i]
            )
        return loss_value
