...
def fx_forward_hook_register(
    module: torch.fx.GraphModule, hook: FxForwardHook, default: bool = True
    ...
    for node in module.graph.nodes:
        if node.op == "get_attr" and node.target == "_fxfh":
            ...
            for hook_call_node in hook_node.users:
                if (
                    hook_call_node.op == "call_function"
                    and hook_call_node.target == wrapped_getattr
                ):
