...
class CenterLoss(_Loss):
    ...
    def __init__(self, num_class : int, output_filter : Optional[Callable[[Any], torch.Tensor]]=None, truth_filter : Optional[Callable[[Any], torch.Tensor]]=None, device : torch.device='cpu'):
        ...
        with torch.no_grad():
            for _, (batch_data, batch_truth) in enumerate(dataloader):
                ...
                for i in range(self.num_class):
                    ...
                    if n_i!=0:
                        self.temp_centers[i].append((torch.mean(out[truth_idx],0),n_i))
    ...
    def forward(self, output : torch.Tensor, truth : torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.num_class):
            if output[truth==i].shape[0]!=0:
                loss_value+=torch.sum(torch.mean((output[truth==i]-self.center[i])**2,0))
                self.temp_centers[i].append((torch.mean(output[truth==i].detach(),0),torch.sum(truth==i)))
        return loss_value