...
from torch.fx.node import Argument, Target, map_arg
...
def infun_node(fx_model: torch.fx.GraphModule, x: torch.fx.Node) -> str:
    ...
    if x.op == "placeholder":
        ...
        return type(
            reduce(lambda x, y: getattr(x, y), x.target.split("."), fx_model)
        ).__name__
    ...
    if isinstance(fx_child_node, torch.fx.Node):
        if isinstance(fx_parent_node, torch.fx.Node):
            if not str(fx_child_node) in nx_graph:
                ...
                if warning:
                    warnings.warn(
                        f"Node {str(fx_child_node)} was not already found in NetworkX graph and was added. torch.fx behaviour is undetermined."
                    )
            if not str(fx_parent_node) in nx_graph:
                ...
                if warning:
                    warnings.warn(
                        f"Node {str(fx_parent_node)} was not already found in NetworkX graph and was added. torch.fx behaviour is undetermined."
                    )
            if shallow:
                nx_graph.add_edge(
                    str(fx_parent_node),
                ...
                nx_graph.edges[(str(fx_parent_node), str(fx_child_node), id)][
                    "label"
                ] = f"Deep {edge_label}\n edge id : {id}"
                nx_graph.deep_dict.setdefault(
                    (str(fx_child_node), edge_label), {}
                ).setdefault(str(fx_parent_node), []).append(id)
        else:
...
def maybe_remove_edge(
    nx_graph: nx.MultiDiGraph, old_arg: Any, nx_child_node: str, label: str
) -> None:
        ...
        if nx_old_node in nx_graph.deep_dict[(nx_child_node, label)]:
            for edge_id in nx_graph.deep_dict[(nx_child_node, label)][nx_old_node]:
                ...
                if default is not None:
                    ...
                    nx_graph.edges[(default, nx_child_node, id)][
                        "label"
                    ] = f"Deep {label}\n edge id : {id}"
                    nx_graph.deep_dict.setdefault(
                        (nx_child_node, label), {}
                    ).setdefault(default, []).append(id)
    ...
    for x in fx_graph.nodes:
        ...
        if node_type != "input":
            for i in range(len(x.args)):
                ...
                if shallow:
                    maybe_add_labeled_edge(
                        nx_graph, x, inputs, edge_label=label, shallow=shallow
                    )
                else:
                    for node in inputs:
                        maybe_add_labeled_edge(
                            nx_graph, x, node, edge_label=label, shallow=shallow
                        )
            for key in x.kwargs:
                label = f"kwarg : {key}"
...
def detect_input(
    arg: Argument,
) -> Tuple[bool, Union[torch.fx.Node, List[torch.fx.Node]]]:
...
class BridgeGraph(object):
    ...
    def to_folder(
        self, folder: Union[str, PathLike], module_name: str = "FxModule"
    ) -> None:
        r"""Refer to torch.fx documentation, torch.fx.GraphModule method of same name"""
    ...
    def add_arg_kwarg(
        ...
        if mode is None:
            if isinstance(position, int):
                ...
                mode = "kwargs"
        assert (
            mode == "args" or mode == "kwargs"
        ), 'Mode of execution is neither "args" nor "kwargs"'
        ...
        if mode == "args":
            ...
            if len(node_args) <= position:
                fx_node.args = (
                    node_args + (None,) * (position - len(node_args)) + (arg,)
                )
            else:
    ...
    def set_args(self, args: Tuple[Argument, ...], nx_node: str) -> None:
        ...
        for i in range(len(args)):
            ...
            if shallow:
                maybe_add_labeled_edge(
                    self.__nx_graph, fx_node, inputs, edge_label=label, shallow=shallow
                )
            else:
                for node in inputs:
                    maybe_add_labeled_edge(
    ...
    def del_edges(
        ...
        if isinstance(default, torch.fx.Node):
            if not str(default) in self.__nx_graph:
                ...
                warnings.warn(
                    f"Node {str(default)} was not already found in NetworkX graph and was added. torch.fx behaviour is undetermined."
                )
        ...
        if position is None:
            assert (
                mode == "args" or mode == "kwargs" or mode is None
            ), 'Mode of execution is not "args", "kwargs" or None'
            if mode == "args" or mode is None:
                child_args = fx_child_node.args
                for i in range(len(child_args)):
                    label = f"arg : {i}"
                    filter_maybe_remove_replace_edge(
                        self.__nx_graph,
                        child_args[i],
                        nx_parent_node,
                        nx_child_node,
                        label,
                        default=to_pass_default,
                    )
                new_args = map_arg(
                    child_args, lambda i: default if i == fx_parent_node else i
                )
            ...
            if mode == "kwargs" or mode is None:
                ...
                new_kwargs = map_arg(
                    child_kwargs, lambda i: default if i == fx_parent_node else i
                )
                fx_child_node.kwargs = new_kwargs
        else:
            assert (
                mode == "args" or mode == "kwargs"
            ), 'Mode of execution is neither "args" nor "kwargs" but a positionnal argument was passed'
            if mode == "args":
                ...
                new_arg = map_arg(
                    child_arg, lambda i: default if i == fx_parent_node else i
                )
                ...
                new_kwarg = map_arg(
                    child_kwarg, lambda i: default if i == fx_parent_node else i
                )
